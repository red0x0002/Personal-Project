...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    /*  XORING THIS WILL CAUSE CRASHES  */

    {

        ID3DBlob* vertexShaderBlob;
        if (FAILED(D3DCompile(c_xor("cbuffer vertexBuffer : register(b0) \
            {\
            ...
            {\
              ...
              output.uv  = input.uv;\
              return output;\
            }"), strlen(c_xor("cbuffer vertexBuffer : register(b0) \
            {\
              float4x4 ProjectionMatrix; \
            };\
            struct VS_INPUT\
            ...
            {\
              ...
              output.uv  = input.uv;\
              return output;\
            }")), NULL, NULL, NULL, c_xor("main"), c_xor("vs_4_0"), 0, 0, &vertexShaderBlob, NULL)))
        ...
        char string1[] = { 'P', 'O', 'S', 'I', 'T', 'I', 'O', 'N', '\x0'};
        char string2[] = { 'T', 'E', 'X', 'C', 'O', 'O', 'R', 'D', '\x0' };
        char string3[] = { 'C', 'O', 'L', 'O', 'R', '\x0' };

        // Create the input layout, XORING THIS WILL CAUSE CRASHES
        D3D11_INPUT_ELEMENT_DESC local_layout[] =
        {
            { string1, 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { string2, 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)IM_OFFSETOF(ImDrawVert, uv),  D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { string3, 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)IM_OFFSETOF(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        };
    ...
    // Create the pixel shader
    {
        ...
        if (FAILED(D3DCompile(c_xor("struct PS_INPUT\
            {\
            float4 pos : SV_POSITION;\
            float4 col : COLOR0;\
            float2 uv  : TEXCOORD0;\
...
bool    ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ...
    io.BackendRendererName = c_xor("imgui_impl_dx11");

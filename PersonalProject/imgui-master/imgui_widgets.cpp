...
{
    return window->GetIDNoKeepAlive(axis == ImGuiAxis_X ? c_xor("#SCROLLX") : c_xor("#SCROLLY"));
}
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImGuiID id = window->GetID(c_xor("#image"));
    PopID();
...
bool ImGui::CheckboxBVM(const char* label, bool* v)
{
    ...
    ImGui::InvisibleButton(label, ImVec2(2 + width, height));
    ...
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.255f, 0.0f, 0.0f, 1.f), ImVec4(0.6f, 0.0f, 0.0f, 1.f), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.255f, 0.0f, 0.0f, 1.f), ImVec4(0.6f, 0.0f, 0.0f, 1.f), t));

    draw_list->AddRectFilled(ImVec2(p.x + 172, p.y + 3), ImVec2(165 + p.x + width, p.y + height - 3), col_bg, height * 2.5f); //Size and position
    draw_list->AddCircleFilled(ImVec2(170 + p.x + radius + t * (width - radius * 2.2f), p.y + 12.6), radius - 4.5f, ImColor(219, 84, 90, 255)); // Size and position
    ...
    if (label_size.x > 0.0f)
    {
        if (*v)
        {
            ...
            RenderText(ImVec2((p.x + 1) + 20, (p.y + height) - 22), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr(c_xor("##ComboPopup"), 0, id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), c_xor("##Combo_%02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar(c_xor("##min"), ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar(c_xor("##max"), ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText(c_xor("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(c_xor("##ColorButton"), col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                // Store current color and open a picker
                g.ColorPickerRef = col_v4;
                OpenPopup(c_xor("picker"));
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(-1, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick(c_xor("context"));
        ...
        if (BeginPopup("picker"))
        {
            ...
            value_changed |= ColorPicker4(c_xor("##picker"), col, picker_flags, &g.ColorPickerRef.x);
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton(c_xor("##current"), col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text(c_xor("Original"));
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(c_xor("##original"), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4(c_xor("##rgb"), col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(c_xor("##hsv"), col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(c_xor("##hex"), col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton(c_xor("##preview"), cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
    if ((flags & ImGuiColorEditFlags_InputRGB) || !(flags & ImGuiColorEditFlags_InputMask_))
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text(c_xor("#%02X%02X%02X\nR: %d, G: %d, B: %d\n(%.3f, %.3f, %.3f)"), cr, cg, cb, cr, cg, cb, col[0], col[1], col[2]);
        else
            Text(c_xor("#%02X%02X%02X%02X\nR:%d, G:%d, B:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)"), cr, cg, cb, ca, cr, cg, cb, ca, col[0], col[1], col[2], col[3]);
    }
    else if (flags & ImGuiColorEditFlags_InputHSV)
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text(c_xor("H: %.3f, S: %.3f, V: %.3f"), col[0], col[1], col[2]);
        else
            Text(c_xor("H: %.3f, S: %.3f, V: %.3f, A: %.3f"), col[0], col[1], col[2], col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_picker && !allow_opt_alpha_bar) || !BeginPopup(c_xor("context")))
        return;
    ImGuiContext& g = *GImGui;
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable(c_xor("##selectable"), false, 0, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags & ImGuiColorEditFlags_PickerMask_);
            SetCursorScreenPos(backup_pos);
            ImVec4 previewing_ref_col;
            memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4(c_xor("##previewing_picker"), &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags(c_xor("Alpha Bar"), &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
}
